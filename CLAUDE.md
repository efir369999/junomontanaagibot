...
> *"Каждое изменение должно быть зафиксировано и распространено."*

**После КАЖДОЙ завершённой задачи я ОБЯЗАН:**

1. **Git commit** с осмысленным комментарием
2. **Синхронизация** по всей сети
...
# 2. Коммит с описанием задачи
git commit -m "TASK: [краткое описание]
...
- **Каждая задача = коммит** — никаких накопленных изменений
- **Осмысленные комментарии** — что сделано, не "fix" или "update"
- **Полная синхронизация** — GitHub + все серверы
- **Немедленно** — сразу после завершения задачи, не "потом"
...
[ ] git commit -m "TASK: ..."
[ ] git push origin main
[ ] ssh sync на все серверы
[ ] Подтвердить синхронизацию
```

**Если я не сделал коммит после задачи — это нарушение протокола.**
